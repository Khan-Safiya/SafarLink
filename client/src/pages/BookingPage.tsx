...
function ETicketCard({ result }: { result: BookingResult }) {
    ...
    return (
        <motion.div
            ...
            className="bg-gradient-to-br from-[#07503E]/10 via-emerald-50 to-white dark:from-emerald-900/30 dark:via-emerald-900/10 dark:to-transparent border-2 border-emerald-200 dark:border-emerald-700/40 rounded-3xl overflow-hidden"
        >
            {/* Header */}
            <div className="bg-[#07503E] px-5 py-4 flex items-center gap-3">
                <CheckCircle2 className="w-6 h-6 text-emerald-300" />
                <div className="flex-1">
                    <p className="text-white font-bold">Booking Confirmed!</p>
                    <p className="text-emerald-200 text-xs">SafarLink E-Ticket</p>
                </div>
                <div className="text-right">
                    <p className="text-white font-mono font-bold text-2xl tracking-widest">{result.pnr}</p>
                    <p className="text-emerald-300 text-xs">PNR</p>
                </div>
            </div>
            ...
            <div className="p-5 space-y-4">
                ...
                {result.segments.map((seg, i) => (
                    <div key={i} className="flex items-center gap-3 bg-white/60 dark:bg-white/5 rounded-2xl px-4 py-3">
                        ...
                        <span className="font-bold text-[#07503E] dark:text-emerald-400 flex items-center text-lg">
                ...
                {/* Payment */}
                <div className="border-t border-emerald-100 dark:border-white/10 pt-3 grid grid-cols-2 gap-2 text-sm">
                    <div>
                        <p className="text-xs text-gray-400">Total Paid</p>
                        <p className="font-bold text-xl text-[#07503E] dark:text-emerald-400 flex items-center">
                ...
                {/* Metro QR */}
                {result.qr_data && (
                    <div className="bg-white dark:bg-white/5 border border-purple-200 dark:border-purple-700/40 rounded-2xl p-4 flex flex-col items-center gap-2">
                        ...
                        <p className="font-mono text-sm tracking-widest text-gray-500">{result.pnr}</p>
                        <p className="text-xs text-gray-400">Valid for single journey · Non-transferable</p>
                    </div>
                )}

                {/* Download */}
                <button className="w-full flex items-center justify-center gap-2 text-sm font-semibold text-gray-500 dark:text-gray-400 border border-dashed border-gray-300 dark:border-white/10 rounded-2xl py-3 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
        ...
        <div className="space-y-4">
            ...
            {phase === "idle" && (
                <motion.div
                    ...
                    className="bg-white dark:bg-card rounded-2xl p-5 border border-amber-200 dark:border-amber-700/40 space-y-4"
                >
            ...
            {phase === "confirming" && (
                <motion.div
                    ...
                    className="bg-white dark:bg-card rounded-2xl p-5 border border-amber-200 dark:border-amber-700/40 space-y-4"
                >
                    <p className="font-bold text-gray-900 dark:text-white text-center">Ready to find a driver?</p>
                    <p className="text-sm text-gray-500 text-center">Drivers within 1 km of your location will be notified.</p>
                    <div className="grid grid-cols-2 gap-3">
                        <button
                            onClick={startSearch}
                            className="bg-[#07503E] hover:bg-[#064031] text-white font-bold py-3 rounded-xl transition-colors"
                        >
                            Yes, Find Driver
                        </button>
                        <button
                            onClick={() => setPhase("idle")}
                            className="border border-gray-200 dark:border-white/10 text-gray-600 dark:text-gray-300 font-semibold py-3 rounded-xl hover:bg-gray-50 dark:hover:bg-white/5 transition-colors"
                        >
                            Cancel
                        </button>
                    </div>
            ...
            {phase === "searching" && (
                <motion.div
                    initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                    className="bg-white dark:bg-card rounded-2xl p-6 border border-blue-200 dark:border-blue-700/40 space-y-5 text-center"
                >
                    <div className="flex justify-center">
                        <div className="relative w-24 h-24">
                            ...
                            <div className="absolute inset-0 flex flex-col items-center justify-center">
                                <span className="text-xl font-bold font-mono text-[#07503E]">{mins}:{secs}</span>
                            </div>
                        </div>
                    </div>
                    <div>
                        <p className="font-bold text-gray-900 dark:text-white">Searching for nearby drivers…</p>
                        <p className="text-sm text-gray-500">Request will expire if no driver accepts within 2 minutes</p>
                    </div>
                    <div className="flex justify-center gap-2">
                        {[0, 1, 2].map(i => (
                            <span key={i} className="w-2 h-2 rounded-full bg-[#07503E] animate-bounce"
            ...
            {phase === "found" && driver && (
                <motion.div
                    ...
                    className="bg-white dark:bg-card rounded-2xl p-5 border-2 border-emerald-400 dark:border-emerald-600 space-y-4"
                >
                    <div className="flex items-center gap-2 text-emerald-600 dark:text-emerald-400">
                        <CheckCircle2 className="w-5 h-5" />
                        <p className="font-bold">Driver Found!</p>
                    </div>
                    <div className="bg-emerald-50 dark:bg-emerald-900/20 rounded-xl p-4 flex items-center gap-4">
                        <div className="w-14 h-14 rounded-2xl bg-[#07503E] flex items-center justify-center text-2xl">
                            🛺
                        </div>
                        <div className="flex-1">
                            ...
                            <div className="flex items-center gap-3 mt-1 text-sm">
                                <span className="flex items-center gap-1 text-amber-500">⭐ {driver.rating}</span>
                                <span className="text-gray-400">·</span>
                                <span className="text-gray-500">{driver.eta} min away</span>
                            </div>
                        </div>
                        <div className="text-right">
                            <p className="text-2xl font-bold text-[#07503E] dark:text-emerald-400 flex items-center">
                    ...
                    {!cancelled ? (
                        <div className="space-y-2">
                            ...
                            <button
                                ...
                                className="w-full border border-red-200 dark:border-red-800/40 text-red-500 font-semibold py-2 rounded-xl text-sm hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
                            >
                                Cancel Ride (₹50 Penalty)
                            </button>
                        </div>
                    ) : (
                        <div className="bg-red-50 dark:bg-red-900/20 rounded-xl px-4 py-3 text-sm text-red-700 dark:text-red-300 font-semibold text-center">
            ...
            {phase === "timeout" && (
                <motion.div
                    initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                    className="bg-white dark:bg-card rounded-2xl p-5 border border-orange-200 dark:border-orange-700/40 space-y-3 text-center"
                >
                    <span className="text-4xl">😔</span>
                    <p className="font-bold text-gray-900 dark:text-white">No Autos Available Nearby</p>
                    ...
                    <button
                        ...
                        className="mt-2 bg-amber-500 hover:bg-amber-600 text-white font-bold py-2.5 px-6 rounded-xl transition-colors"
                    >
                        Try Again
                    </button>
                </motion.div>
    ...
    return (
        <div className="min-h-screen bg-[#F4FDF7] dark:bg-background font-sans">
            ...
            <main className="max-w-2xl mx-auto px-4 py-8 space-y-6">
                ...
                {/* Back button */}
                <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-sm text-gray-500 hover:text-[#07503E] transition-colors">
                    <ArrowLeft className="w-4 h-4" /> Back
                </button>

                {/* Header */}
                <div className="bg-white dark:bg-card rounded-3xl p-5 border border-gray-100 dark:border-white/10 shadow-sm">
                    ...
                    <div className="flex gap-3 mt-3 text-xs text-gray-500">
                        <span className="capitalize bg-emerald-50 dark:bg-emerald-900/20 text-[#07503E] dark:text-emerald-400 font-semibold px-3 py-1 rounded-full">
                            {routeType} route
                        </span>
                ...
                {/* Tab selector */}
                <div className="flex gap-2 bg-white dark:bg-card rounded-2xl p-1.5 border border-gray-100 dark:border-white/10 shadow-sm">
                    {showBus && (
                        <button
                ...
                <AnimatePresence mode="wait">
                    ...
                    {activeTab === "bus" && (
                        <motion.div key="bus" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} className="space-y-4">
                            {busPhase === "idle" && (
                                <div className="bg-white dark:bg-card rounded-2xl p-5 border border-red-100 dark:border-red-900/30 space-y-3">
                                    <div className="flex items-center gap-3">
                                        <span className="text-3xl">🚌</span>
                                        <div>
                                            <p className="font-bold text-gray-900 dark:text-white">Book PMPML Bus Ticket</p>
                                            <p className="text-sm text-gray-500">Fixed fare · Valid for 1 journey</p>
                                        </div>
                                        <span className="ml-auto text-2xl font-bold text-[#07503E] dark:text-emerald-400 flex items-center">
                            ...
                            {busPhase === "processing" && (
                                <div className="bg-white dark:bg-card rounded-2xl p-8 border text-center space-y-3">
                    ...
                    {activeTab === "metro" && (
                        <motion.div key="metro" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} className="space-y-4">
                            {metroPhase === "idle" && (
                                <div className="bg-white dark:bg-card rounded-2xl p-5 border border-purple-100 dark:border-purple-900/30 space-y-3">
                                    <div className="flex items-center gap-3">
                                        <span className="text-3xl">🚇</span>
                                        <div>
                                            <p className="font-bold text-gray-900 dark:text-white">Book Pune Metro Ticket</p>
